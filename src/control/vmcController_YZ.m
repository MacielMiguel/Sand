...
try
    ...
    %% === VMC parameters ===
    % Kvmc remains aligned with the Cartesian axes [Y, Z]
    Kvmc = diag([40 30]); % Tuning parameters
    Dvmc = diag([2  3]);
    tauMax = [5.0; 5.0]; 
    ...
    q0 = [-2.206; -0.449]; % The ellipse will start from this point
    ...
    while true
        ...
        q_corr = q_read(:) + [theta2_offset; theta3_offset];
    ...
    theta2 = q_read(1) + theta2_offset; 
    theta3 = q_read(2) + theta3_offset; 
    ...
    chi_d_plot = zeros(2,numel(tau_plot));
    for k = 1:numel(tau_plot)
        chi_d_plot(:,k) = chi0 + [a*cos(omega*tau_plot(k)); b*sin(omega*tau_plot(k))];
    end
    plot(chi_d_plot(2,:), chi_d_plot(1,:), '--');   % ideal ellipse
    h_real = plot(chi(2), chi(1), 'o-', 'Color', [1 0.5 0]);
    legend('show'); 
    ...
    while true
        ...
        % --- Jacobian ---
        dphi_dth3 = -(L3*(L2*cos(theta3) + L3)) / (m^2);
        
        J = [ 0, -(L2*L3*sin(theta3))/y;
              -m*cos(phi), -(L2*L3*sin(theta3))/m*sin(phi) + m*cos(phi)*dphi_dth3 ];
        
        dchi = J * dq;
    
        % --- Desired trajectory ---
        chi_d  = chi0 + [a*cos(omega*t); b*sin(omega*t)];
        dchi_d =       [-a*omega*sin(omega*t); b*omega*cos(omega*t)];
        ...
        % --- Update plot ---
        set(h_real, 'XData', [get(h_real,'XData') chi(2)], ...
                    'YData', [get(h_real,'YData') chi(1)]);
        drawnow("limitrate");
    
        % --- Debug ---
        fprintf("t=%.3f | dt=%.4f | LatTau=%.2f LegTau=%.2f | Y_err=%.3f\n", ...
                t, dt_meas, tau(1), tau(2), e(1));
